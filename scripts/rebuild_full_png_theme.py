...
# Map: Category -> { Physical Target Name: Core Source Name }
COPY_MAPPINGS = {
    "places": {
...
print("=== Rebuilding Rich Standalone Icon Theme ===")
...
# 6. Generate physical file copies (no symlinks!)
print("\n=== Generating Physical Copies for Coverage ===")
copied_count = 0
for category, mappings in COPY_MAPPINGS.items():
    for link_name, src_name in mappings.items():
        for size in SIZES:
            src_path = os.path.join(THEME_DIR, f"{size}x{size}", category, src_name)
            dest_path = os.path.join(THEME_DIR, f"{size}x{size}", category, link_name)
            
            if os.path.exists(src_path):
                # Copy the file physically
                shutil.copy2(src_path, dest_path)
                copied_count += 1
        print(f"Created copies: {link_name} -> {src_name}")

print(f"\nGenerated {copied_count} total physical copies across all size folders.")
